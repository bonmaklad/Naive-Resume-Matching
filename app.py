...
if len(Jobs['Name']) <= 1:
    st.write(
        "There is only 1 Opportunity present. It will be used to create scores.")
else:
    st.write("There are ", len(Jobs['Name']),
             "Opportunities available. Please select one.")

    st.write("There are ", len(Resumes['Name']),
             "talented individuals that matched.")

# Asking to Print the Job Desciption Names
if len(Jobs['Name']) > 1:
    option_yn = st.selectbox(
        "Show the Opportunity Titles?", options=['YES', 'NO'])
    if option_yn == 'YES':
        index = [a for a in range(len(Jobs['Name']))]
        fig = go.Figure(data=[go.Table(header=dict(values=["Number", "Opportunity Title"], line_color='white',
                                                   fill_color='Black', font_color="white",),
...
index = st.slider("Which Opportunity to select ? : ", 0,
                  len(Jobs['Name'])-1, 1)


option_yn = st.selectbox("Show the Opportunity ?", options=['NO', 'YES'])
if option_yn == 'YES':
    st.markdown("---")
...
fig1 = go.Figure(data=[go.Table(
    header=dict(values=["Rank", "Name", "Cost", "Location", "Score", "Message"],
                fill_color='black',
                align='center', font=dict(color='white', size=16)),
    cells=dict(values=[Ranked_resumes.Rank, Ranked_resumes.Name, "$110", "Wellington", Ranked_resumes.Scores, "False"],
               fill_color='lightyellow',
               align='left'))])

fig1.update_layout(title="Top Ranked Talent", width=700, height=1100)
st.write(fig1)
